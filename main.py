...
if __name__ == "__main__":
    ...
    try:
        ...
        #set the number of in channels based on the max conditioning channels
        if 'just_captions' in config.model.params and config.model.params.just_captions==True:
            config.model.params.cond_stage_key='caption'
            config.model.params.cond_stage_config.target='ldm.modules.encoders.modules.FrozenCLIPEmbedder'
            config.data.params.max_number_of_conditioning_frames = 0
        if 'ffp' in config.model.params and config.model.params.ffp==True:
            config.data.params.max_number_of_conditioning_frames = 0

        config.model.params.unet_config.params.in_channels = config.data.params.max_number_of_conditioning_frames*2*4+8
            ...
            if in_shape != in_filters_load.shape:
                ...
                for input_key in input_keys:
                    ...
                    if input_weight.size() != old_state[input_key].size():
                        ...
                        input_weight.zero_()
                        min_dim = min(input_weight.shape[1], old_state[input_key].shape[1])
                        input_weight[:, :min_dim, :, :].copy_(old_state[input_key][:,:min_dim,:,:])
